...
export function build() {
  ...
  */
  ...
  /*
    ...
    左に輪、右に無地の札。**札があるせいで、絵全体の中心と輪の中心がずれる。**
    自動の切り取りは絵の外接矩形で切るので、切ったあとも輪は左に寄ったまま。

    これが要るのは、写真を「画面の中心」に置く作りだと、
    こういうフレームで顔が穴の外に落ちるから。落ちることを数えるための1枚。
  */
  write(
    'offset-hole.png',
    png(S, S, (x, y) => {
      const inBox = (x0, y0, x1, y1) => x >= x0 && x <= x1 && y >= y0 && y <= y1;
      const cx = S * 0.3;
      const cy = S * 0.5;
      const R = S * 0.24;
      const TH = S * 0.06;
      ...
      // 左の輪
      const rd = Math.abs(Math.hypot(x - cx, y - cy) - R);
      if (rd < TH / 2) col = over([20, 20, 28], col, Math.min(1, (TH / 2 - rd) / 2));
      // 右の札。これが外接矩形を右へ広げる
      if (inBox(S * 0.62, S * 0.42, S * 0.9, S * 0.58)) col = [20, 20, 28];
      return [...col, 255];
    }),
  );
  ...
  */
  write(
    ...
    png(S, S, (x, y) => {
      const d = Math.hypot(x - C, y - C);
      if (d < S * 0.12) return [230, 30, 30, 255]; // まん中の印（赤）
      return [40, 60, 200, 255]; // まわり（青）
    }),
  );
