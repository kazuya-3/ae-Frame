...
export function build() {
  ...
  */
  ...
  /*
    ...
    生成AIで作った動画にいちばん多い形（発光を保つために黒で作る）を、
    小さく再現したもの。にじむ光と、離れて浮かぶ粒が入っている。
    ここで見たいのは「粒が残るか」ではなく、**裾が半透明のまま残るか**。
    裾が切り落とされると、光は光に見えなくなる。
  */
  write(
    'studio-glow.png',
    png(480, 270, (x, y) => {
      let v = 9; // 地。完全な黒ではなく、撮影と同じで少しだけ持ち上がっている
      const glow = (cx, cy, r, peak) => {
        const d = Math.hypot(x - cx, y - cy);
        v += peak * Math.exp(-(d * d) / (2 * r * r));
      };
      glow(200, 140, 38, 250); // まん中のにじむ光
      for (const [sx, sy] of [
        [300, 110],
        [318, 132],
        [286, 150],
        ...
        glow(sx, sy, 2.4, 255); // 浮かぶ粒
      const c = clamp(v);
      return [c, clamp(c * 0.86), clamp(c * 0.45), 255]; // 金色の光
    }),
  );
