...
export function analyze(data: ImageData): Analysis {
  ...
  */
  ...
  /*
    ...
    ここは「白い背景の上では、うすい光とうすい絵の具は同じ色になる」という
    避けようのない曖昧さに触れている。幅を 0.3 にすると、白との色差が 0.31 未満の色を
    まとめてうすくする。ネオンの光にはそれが要るが、花火と淡い水彩が同居するような
    デザインでは、絵のほうまで巻き込んで透けさせてしまう。

    どちらを既定にするかは、失敗したときの気づきやすさで決めた。
    消え残りは画面を見ればすぐ分かり、スライダーでも筆でも直せる。
    一方、絵がうっすら透けたのは気づきにくく、書き出したあとでは直せない。
    だから既定は「消しすぎない」側に倒し、光を階調で残したい人が
    「光のにじみを残す」を上げる、という順番にしている。

    hasGlow は、この幅を自動で上げるためには使わない（上の理由）。
    それでも画像の性格を表す指標として返しており、囲まれた部分の扱いなど
    別の判断に使えるように残してある。
  */
  const suggestedSoftness = 0.01;
