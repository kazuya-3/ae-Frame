 ...
 * 書き出す画像の一辺。
 *
 * 下の `Placement` の x と y は、**この座標系での画素**。
 * ここを変えると、すでに配ってあるフレームの置き場所が全部ずれる。
 * 変えるなら `v` を上げて、古い版を読み替える手当てが要る。
 */
 ...
 * ── 単位について（前に書いたことの訂正）──
 *
 * ここに「比で持つ」と書いていたが、**実際は画素**（上の EXPORT_SIZE 基準）。
 * 書いたことと中身が違っていた。いま配ってあるフレームが画素で入っているので、
 * 比に直すと**過去に配ったものがずれる**。直すのは単位ではなく、この説明のほう。
 */
