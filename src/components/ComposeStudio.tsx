  ...
  */
  useEffect(() => {
    ...
    */
    ...
    setTarget('photo');
    /*
      もらったフレームでは、写真を**まるく切りぬいた状態から始める**。

      ── 何が起きていたか ──

      アイコンフレームはたいてい輪。輪の外側はとうめいなので、四角い写真を
      そのまま敷くと、**写真の角が輪の外に出て、直線のフチが見える**。
      「すきまの色」をとうめいにして配ったフレームだと、輪の外の地に
      写真の四角がそのまま乗る。仕上がりに見えない。

      配った人はその状態を見ていない（配るときに写真は無い）ので、
      気づけるのはもらった人だけ。そのもらった人に直す手段が無かった。

      まるを初期値にすると、角は最初から無い。**押す回数は増えない。**
      輪でないフレームのために、下の「写真のかたち」で戻せるようにしてある。
    */
    setShape('circle');
      ...
      <div className="stack">
        ...
        */}
        ...
        {/*
          ...
          等間隔だと 100% がレールの 22% のところに来て、
          「小さくする」が左端の 1/5 に押しこめられる。
          枠の内側に写真を収めたい人がいちばん使うのはそこなので、
          狙えないまま「−」を何回も押すことになっていた。
        */}
        <Slider
        ...
        {/*
          ...
          ここには「この2つは**フレームの見え方**の一部で、配った人が
          決めるべきもの」と書いて、もらった人には出していなかった。
          **間違い。** 切りぬかれるのは配った人のフレームではなく、
          **もらった人自身の写真**。他人が決めていいものではない。

          そのせいで、輪のフレームをもらった人は、自分の写真の角が
          輪の外にはみ出したまま直せなかった。上の lock のところで
          まるを初期値にしたが、それだけでは輪でないフレームで詰まる。
          **戻す道が要る。**

          「どこを切りぬくか」も同じ理由で要る。縦長の写真をまるく抜くと
          残るのはまん中の帯で、全身写真なら顔が落ちる。指でうごかしても
          窓の中身は変わらない（型 Crop の説明）ので、これが唯一の手段。
          切りぬく形を選んだ人にしか出ないので、並ぶ項目は増えない。
        */}
        <div className="field">
        ...
        */}
        ...
        {/*
          ...
          こちらが渡すのはアルファチャンネル付きの PNG そのもので、
          バイト列は加工されない。壊れるのは**受け取ったアプリの中**。

          多くの SNS・メッセージアプリは、受け取った画像を JPEG に変換する。
          JPEG にアルファチャンネルは無いので、とうめいだったところは
          白か黒で埋まる。銀色の枠のつもりで渡したものが、
          黒い四角になって届く、ということが起きる。

          受け取り側は制御できない。API も無い。
          できるのは**渡す前に伝える**ことだけなので、ボタンのそばに書く。
